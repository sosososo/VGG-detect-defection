...
class MYVGG(nn.Module):
    ...
    def __init__(self, features, num_classes=2):
        ...
        self.lastclassifier = nn.Sequential(
            nn.Linear(4096,  num_classes),
        )
...
cfg = {
    ...
    'F': [64, 'M', 128, 'M', 256, 'M', 512, 'M', 512, 512, 'M'],
    'G': [64, 'M', 128, 'M', 256, 256 ,'M', 128, 'M'],
...
def vgg9(pretrained=False, **kwargs):
    ...
    """
    model = MYVGG(make_layers(cfg['F']), **kwargs)
    # print('VGG 11 model:')
    # print(model)
    if pretrained:
...
def vgg11_bn(pretrained=False, **kwargs):
    ...
    if pretrained:
        if model.num_classes!=1000:
            pretrained_dick=torch.load(model_param_location['vgg11_bn'])
            model_dict=model.state_dict()
            pretrained_dick={k:v for k,v in pretrained_dick.items() if k in model_dict}
            model_dict.update(pretrained_dick)
            model.load_state_dict(model_dict)
        else:
            model.load_state_dict(torch.load(model_param_location['vgg11_bn']))
...
def vgg13_bn(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MYVGG(make_layers(cfg['B'], batch_norm=True), **kwargs)
    if pretrained:
        model.load_state_dict(torch.load(model_param_location['vgg13_bn']))
    return model
...
def vgg16_bn(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MYVGG(make_layers(cfg['D'], batch_norm=True), **kwargs)
    if pretrained:
        model.load_state_dict(torch.load(model_param_location['vgg16_bn']))
    return model
...
def vgg19_bn(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MYVGG(make_layers(cfg['E'], batch_norm=True), **kwargs)
    if pretrained:
        model.load_state_dict(torch.load(model_param_location['vgg19_bn']))
    return model
